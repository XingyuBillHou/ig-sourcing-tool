...
def _extract_searchable_ad_text(item: dict) -> str:
    ...
    _walk(item.get("snapshot"))
    for key in ("title", "body", "adCreativeBody", "linkUrl", "linkDescription", "caption"):
        val = item.get(key)
        if isinstance(val, str) and val.strip():
...
def _ad_matches_keyword(item: dict, keyword: str, *, min_ratio: float = 1.0) -> bool:
    """判断广告文案/品牌是否与关键词相关。"""
    ...
    if kw and kw in text:
        return True
    matched = sum(1 for token in tokens if _token_in_text(token, text))
    if min_ratio >= 1.0:
        required = len(tokens)
    else:
        required = max(1, int(len(tokens) * min_ratio + 0.999))
...
def filter_top_impression_videos(
    ...
    核心筛选逻辑：
    1. 过滤出含视频链接的广告
    2. 仅保留近 lookback_days 天内开始投放的广告（避免多次抓取重叠）
    3. 按关键词匹配广告文案/品牌（剔除无关结果）
    4. 按曝光量（高→低）排序，取 top_n
    """
    all_video_ads = []
    for item in items:
    ...
    video_ads = [
        ad for ad in all_video_ads if _is_ad_started_within_lookback(ad["raw"], lookback_days)
    ]

    stats = {
        "total_items": len(items),
        "total_video": len(all_video_ads),
        "within_lookback": len(video_ads),
        "lookback_days": lookback_days,
        "keyword": (search_keyword or "").strip(),
        "keyword_matched": len(video_ads),
        "match_mode": "all",
        "with_impression_data": sum(1 for ad in video_ads if ad["impression_score"] >= 0),
        "sort_by": "impression_desc",
    }

    if not video_ads:
        stats["match_mode"] = "none_in_window" if all_video_ads else "none"
        return [], stats

    kw = (search_keyword or "").strip()
    if kw:
        strict = [ad for ad in video_ads if _ad_matches_keyword(ad["raw"], kw, min_ratio=1.0)]
        if strict:
            video_ads = strict
            stats["keyword_matched"] = len(strict)
            stats["match_mode"] = "strict"
        else:
            relaxed = [ad for ad in video_ads if _ad_matches_keyword(ad["raw"], kw, min_ratio=0.5)]
            if relaxed:
                video_ads = relaxed
                stats["keyword_matched"] = len(relaxed)
                stats["match_mode"] = "relaxed"
            else:
                stats["keyword_matched"] = 0
                stats["match_mode"] = "none"
                return [], stats

    stats["with_impression_data"] = sum(1 for ad in video_ads if ad["impression_score"] >= 0)
...
with st.sidebar:
    ...
    with st.expander("🎬 Gemini 视频模式（可选，需 VPN）", expanded=False):
...
with tab_fetch:
    ...
    with col_kw:
        search_keyword = st.text_input(
            ...
            help=(
                "在 Meta Ad Library 全库中按关键词搜索视频广告。"
                f"仅保留近 {FETCH_LOOKBACK_DAYS} 天内开始投放的广告，按曝光量取 Top 3。"
            ),
            key="fetch_keyword",
        )
    with col_country:
    ...
    fetch_button = st.button(
        f"📥 按近{FETCH_LOOKBACK_DAYS}天曝光量捞 Top 3 并下载",
        type="primary",
        use_container_width=True,
    )
    ...
    if fetch_button:
        ...
        progress.set_step(0, "正在连接 Apify，通常需 1~5 分钟")
        progress.update(0.05, "Apify 抓取广告库", f"关键词：{search_keyword.strip()} · 地区：{country}")

        try:
            raw_items = fetch_fb_ads(apify_token, search_keyword.strip(), country)
            progress.complete_step(0)
            progress.set_step(1)
            progress.update(
                0.35,
                f"筛选近{FETCH_LOOKBACK_DAYS}天曝光最高的视频",
                f"共抓取 {len(raw_items)} 条",
            )

            if not raw_items:
                st.warning(f"⚠️ 未抓取到关键词 “{search_keyword}” 的任何广告。")
                st.stop()

            top_ads, kw_stats = filter_top_impression_videos(
                raw_items, TOP_N, search_keyword.strip()
            )
            if not top_ads:
                diag = diagnose_ad_items(raw_items)
                if kw_stats.get("match_mode") == "none_in_window":
                    st.warning(
                        f"⚠️ 共抓取 {len(raw_items)} 条广告（其中 {diag['with_video_url']} 条含视频），"
                        f"但没有「近 {FETCH_LOOKBACK_DAYS} 天内开始投放」的视频广告。"
                        f"（共 {kw_stats.get('total_video', 0)} 条视频广告均在更早日期开始）"
                    )
                    st.info(
                        f"建议：放宽时间范围、换关键词，或等近 {FETCH_LOOKBACK_DAYS} 天内有新广告上线后再试。"
                    )
                elif kw_stats.get("match_mode") == "none":
                    st.warning(
                        f"⚠️ 共抓取 {len(raw_items)} 条广告（其中 {diag['with_video_url']} 条含视频，"
                        f"近 {FETCH_LOOKBACK_DAYS} 天内 {kw_stats.get('within_lookback', 0)} 条），"
                        f"但没有广告文案/品牌与关键词「{search_keyword.strip()}」匹配。"
                    )
                    st.info(
                        "建议：换更具体的关键词（如 product + category）、检查拼写，"
                        "或先在 Meta Ad Library 网页确认该词能搜到相关视频广告。"
                    )
                    with st.expander("查看部分抓取品牌（供排查）"):
                        st.write(", ".join(diag.get("sample_page_names") or ["无"]))
                else:
                    st.warning(f"⚠️ 未找到可下载的视频广告（共 {diag['total']} 条）。")
                st.stop()

            match_note = ""
            if kw_stats.get("match_mode") == "relaxed":
                match_note = "（部分词匹配）"
            imp_note = ""
            if kw_stats.get("with_impression_data", 0) == 0:
                imp_note = "（抓取结果无曝光数据，已按投放时长作为次要排序）"
            elif kw_stats.get("with_impression_data", 0) < len(top_ads):
                imp_note = f"（{len(top_ads) - kw_stats.get('with_impression_data', 0)} 条无曝光数据，排在后面）"
            progress.update(
                0.38,
                f"筛选近{FETCH_LOOKBACK_DAYS}天关键词相关视频",
                f"近 {FETCH_LOOKBACK_DAYS} 天内 {kw_stats.get('within_lookback', 0)} 条视频 · "
                f"关键词「{search_keyword.strip()}」匹配 {kw_stats.get('keyword_matched', 0)} 条 · "
                f"有曝光数据 {kw_stats.get('with_impression_data', 0)} 条 · "
                f"按曝光量取 Top {len(top_ads)}{match_note}{imp_note}",
            )
            ...
            for idx, ad in enumerate(top_ads, start=1):
                ...
                progress.update(
                    0.4 + (idx - 1) / len(top_ads) * 0.45,
                    f"[{idx}/{len(top_ads)}] 下载视频",
                    ad["video_url"][:100],
                )
                try:
                    local_path = download_video(ad["video_url"], idx)
                    downloaded_ads.append(
                        {
                            "index": idx,
                            "brand_name": brand_name,
